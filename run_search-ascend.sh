...
if $DOCKER_COMPOSE_CMD -p user -f docker-compose-linux-search-ascend.yaml down |& tee /dev/tty | grep -q "services.qanything_local.deploy.resources.reservations value 'devices' does not match any of the regexes"; then
    echo "检测到 Docker Compose 版本过低，请升级到v2.23.3或更高版本。执行docker-compose -v查看版本。"
fi
mkdir -p volumes/es/data
chmod 777 -R volumes/es/data
$DOCKER_COMPOSE_CMD -p user -f docker-compose-linux-search-ascend.yaml up -d
$DOCKER_COMPOSE_CMD -p user -f docker-compose-linux-search-ascend.yaml logs -f qanything_local
# 检查日志输出
